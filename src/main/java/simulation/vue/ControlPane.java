...
import simulation.chargeur.PredefinedTopologies;
import simulation.export.SimulationCsvExporter;
...
public class ControlPane extends VBox {
    ...
    private FlowPane buildRow2() {
        ...
        comboTopo.getItems().addAll("— choose —","Dense Forest","River in Forest","City with Courtyard");
        comboTopo.getSelectionModel().selectFirst();
        comboTopo.setOnAction(e -> {
            int idx = comboTopo.getSelectionModel().getSelectedIndex();
            if (idx == 0) return;
            Grid g = switch (idx) {
                case 1  -> PredefinedTopologies.denseForest();
                case 2  -> PredefinedTopologies.riverInForest();
                default -> PredefinedTopologies.cityWithCourtyard();
            };
            stopTimer();
